...
#define turndeg_kp (20.0f)
#define turndeg_ki (1.0f)
#define turndeg_kd (350.0f)
#define DECELERATION_KP  (40)
#define ORIENT_RIGHT_KP  (150.0f)
#define ORIENT_RIGHT_KD  (4000.0f)
...
#define TASK_15_WAIT_DURATION        (2.0)
...
long error_sum = 0.0;
float wallrange = 0.0;
float case1_Lspeed = 130.0;
float case1_Rspeed = 130.0;
...
void changeControllers(int _stage, ros::ServiceClient* _funcase_client,ros::ServiceClient* _set_IMU_zero,
  ...
  case 1:
    //move farward
    printf("L -> %f, R -> %f\n",case1_Lspeed,  case1_Rspeed);
    moveit_msg.data.push_back(case1_Lspeed);
    moveit_msg.data.push_back(case1_Rspeed);
    pubmsg_enable = true;
    break;
  ...
  case 2:
    ...
    error_back= error;
    error_sum += error;
    turn = static_cast<int16_t>(turndeg_kp*error + turndeg_kd*error_dot + turndeg_ki * error_sum);
  ...
  case 171:
    ...
    turn = static_cast<int16_t>(ORIENT_RIGHT_KP*error + ORIENT_RIGHT_KD*error_dot);
    moveit_msg.data.push_back(40+turn);
    moveit_msg.data.push_back(40-turn+10);
    pubmsg_enable = true;
    break;
  ...
  case 18:
    ...
    switch_control.request.start_controllers.push_back("track_line_controller");
    switch_enable = true;
    SetLineDynamicParams(&dynamic_msg, 0.2,0.0,4.7,90.0);
    dynamic_srv.request.config = dynamic_msg;
    dyline_enable = true;
  ...
  case 19:
    if(isnan(front_length)){
      ...
      turn = static_cast<double>((front_length) * 50);
    }
    speed = 150.0;
    printf("turn -> %d\n", turn);
    printf("speed -> %f\n", speed);
...
bool stage_change_detect(int _stage){
  ...
  case 1:
    ...
    if(fg_usetimer){
      if((sensor_value[6] == 2)){
        case1_Lspeed = -case1_Lspeed;
        case1_Rspeed = case1_Rspeed;        
      }
      else if((sensor_value[6] == 1)){
        case1_Lspeed = case1_Lspeed; 
        case1_Rspeed = -case1_Rspeed;
      }
      else if((sensor_value[6] == 0)){
        case1_Lspeed = case1_Lspeed; 
        case1_Rspeed = case1_Rspeed;
      }else{
        case1_Lspeed = -case1_Lspeed; 
      ...
      if((ros::Time::now().toSec() - last_time.toSec() > 1.0f) &&(sensor_value[6] == 3)){
        last_time = ros::Time::now();
        return true;
      }
    }
...
//      return true;
    ...
    if(fg_usetimer){
      if((ros::Time::now().toSec() - last_time.toSec() > 2.0) && (laser_distence_overlimit_conter > 2 )){
        fg_usetimer = false;
        last_time = ros::Time::now();
...
//        if(get_sensor_average() > 200)
  ...
  case 32:
    ...
    if((sensor_value[6] == 1) && (sensor_value[5] < 30))
      return true;
