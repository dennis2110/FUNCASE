...
int main(int argc, char** argv)
{
  ...
  ros::init(argc, argv, "read_serial_test");
  ...
  while(ros::ok())
  {
    try
    {
      if (ser.isOpen())
      {
        ...
        int size_write = ser.write(TwoPWM, 7);
        ROS_INFO("write:%d", size_write);
        ...
        if(ser.available())
        {
          ...
          while (input.length() >= 5) {
            ...
            {
              if (input.length() >= data_packet_start + 4)  //check if positions 26,27 exist, then test values
              {
              ...
              {
                if (input.length() >= data_packet_start + 4)
                {
